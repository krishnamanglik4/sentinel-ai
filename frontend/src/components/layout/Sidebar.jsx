...
export const Sidebar = () => {
  const menuItems = [
    ...
    { label: 'How It Works', path: '/about', icon: HelpCircle },
    { label: 'Settings', path: '/app/settings', icon: Settings },
  ];

  return (
    <aside className="w-64 border-r border-slate-800/80 bg-[#080c14] flex flex-col justify-between hidden md:flex min-h-[calc(100vh-4rem)] sticky top-16 shrink-0">
      ...
      {/* Footer Banner */}
      <div className="p-4">
        <div className="rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-950/30 via-slate-900 to-violet-950/30 p-3.5 backdrop-blur-md">
          <div className="flex items-center gap-2 text-cyan-400 text-xs font-bold mb-1">
            <ShieldCheck className="w-4 h-4" />
            <span>AI Risk Engine v1.0</span>
          </div>
          <p className="text-[11px] text-slate-400 leading-relaxed">
            Normalized 0–100 forensic signals & XAI explanation pipeline.
          </p>
        </div>
      </div>
    </aside>
